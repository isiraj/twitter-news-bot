...
def send_mail(tweets):
    ...
    body = f""
    # This for loop appends rows to the email body that contains the username of who sent the tweet, the text of the tweet, and the link of the tweet.
    for tweet in tweets:
        body += f"{tweet.user.name}: {(tweet.text)} (https://twitter.com/i/web/status/{tweet.id_str})\n"
